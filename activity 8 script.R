...
dirR <- "/Users/kristenbitsberger/Documents/a08/oneida"
...
#######question 4########
rgbS1 <- stack(rdatB4, rdatB3, rdatB2, rdatB8)/1000
plotRGB(rgbS1, r = 4, g = 1, b = 2, stretch="lin", maxpixels=rgbS1@ncols*rgbS1@nrows)

##raster stack with three bands of light that you want to use 
...
#######question 8########
rasterNDVI <-  data.frame(landcID = as.factor(rep(c("algae","water","agri","forest","wetland"),each=120)),
                           x=c(algae@coords[,1],water@coords[,1],agri@coords[,1],forest@coords[,1],wetlands@coords[,1]),
                           y=c(algae@coords[,2],water@coords[,2],agri@coords[,2],forest@coords[,2],wetlands@coords[,2]))

rasterNDVI$NDVI <- raster::extract(NDVI,landExtract[,2:3])
...
#######question 9#######
in.mod <- lm(rasterNDVI$NDVI ~ rasterNDVI$landcID)
in.aov <- aov(in.mod)
summary(in.aov)
...
tapply(rasterNDVI$NDVI, rasterNDVI$landcID, "mean")
